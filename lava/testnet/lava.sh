...
##########################################################
# Main
project_name="Lava-network"
repo="https://github.com/lavanet/lava"
repo_dir="lava"
bin_name="lavad"
chain_dir="$HOME/.lava"
...
chain[1]="Testnet"
denom[1]="ulava"
chain_id[1]="lava-testnet-1"
repo_tag[1]="0.4.0-rc2"
rpc[1]="https://t-lava.rpc.utsa.tech:443"
...
for v in ${!chain_id[@]}; do
    if [[ $($bin_name status 2>/dev/null | jq -r '.NodeInfo.network' 2>/dev/null) == ${chain_id[$v]} ]]; then
        ...
        if [[ -n ${miss_block} ]]; then
            ...
            echo -e "Node as validator : $check_mark True"
            if [[ $val_status =~ "BONDED" ]]; then
                echo -e "Validator status  : $check_mark Active"
            else
                echo -e "Validator status  : $x_mark Inactive"
            fi
            echo -e "Validator moniker : $val_moniker"
            echo -e "Missed blocks     : $miss_block"
            echo -e "Jailed            : $val_jailed"
        else
            echo -e "Node as validator : $x_mark False"
